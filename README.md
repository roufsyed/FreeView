...
- **Share, paste, or tap** - share a Medium link from any app, paste it into the in-app URL bar, or tap a Medium link (any domain) and choose FreeView
- **Reading history** - optionally keep your last 20 / 50 / 100 articles and reopen them with a tap
- **Bookmarks** - save articles to a dedicated list, delete with long-press multi-select, or bookmark straight from History
- **Settings** - reader service, theme (light / dark / system), article text size, clear cache & cookies, and set FreeView as the default Medium handler
...
2. **Share** its link to FreeView, **paste** it into the top bar, or **tap** a Medium link and pick FreeView.
3. FreeView rewrites the link for your selected service and opens the full article.
4. Switch the service (and more) any time in **Settings**, and revisit past reads from **History** or your saved **Bookmarks**.

> Tapping works for Medium articles on **any** domain - `medium.com`, its subdomains, and custom
> publication domains (towardsdatascience.com, ai.plainenglish.io, …). FreeView is offered in the
> "Open with" chooser for web links, so pick it per-link rather than making it your default browser.
...
| Storage       | SharedPreferences (settings, history, bookmarks) |
